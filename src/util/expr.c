 ...
 *
 * Left open: the checker's token model has no way to express "this value
 * vacuously satisfies a linear token without a real release call", so it
 * still flags several `if (x != oom_sentinel) free(x)` sites below as
 * unfreed/not-owned along the oom_sentinel path. Not fixable without a new
 * annotation or reworking this file's OOM-signaling design. */
...
/* a/b left unmarked: consume(heap_allocated), always freed whether or not read. */
withtok(heap_allocated)
...
/* a/b left unmarked, same as do_arith(); strcmp(a, b) below is the one
 * place this function reads them, only when neither is numeric. */
withtok(heap_allocated)
	...
	if (re.re_nsub >= 1) {
		if (matched && pm[1].rm_so >= 0) {
			...
			if (!result) { xerr(c, "out of memory"); result = dupstr(c, ""); }
				...
				/* Safe by regexec()'s postcondition: pm[1] is a sub-match
				 * of `a`, so 0 <= rm_so <= rm_eo <= strlen(a). The checker
				 * still flags this (no extent contract on regexec()). */
