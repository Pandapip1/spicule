...
/* returns_nonnull holds because the only alloc-failure path below calls exit(2) first. */
...
/* path is always one of nftw()'s callback pathnames, hence null-terminated. */
...
/* n is never NULL here: the parser only ever builds N_NOT/N_AND/N_OR
 * nodes with both children set, unlike free_node()/has_action()/
 * flush_plus() below, which guard against it anyway. */
...
/* path is nftw()'s callback pathname, POSIX-guaranteed null-terminated. */
...
/* Flush every "{} +" node's accumulated batch in size-bounded chunks (see
 * header comment on why this is deferred to the end of the walk). */
